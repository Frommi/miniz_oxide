...
use miniz_oxide::inflate::{self, TINFLStatus, TINFL_LZ_DICT_SIZE, inflate_flags, tinfl_decompressor};
...
pub fn mz_inflate_oxide(stream_oxide: &mut StreamOxide<inflate_state>, flush: c_int) -> MZResult {
    ...
    loop {
        let status = {
            ...
            inflate::decompress(&mut state.m_decomp, *next_in, &mut out_cursor, decomp_flags)
        };
