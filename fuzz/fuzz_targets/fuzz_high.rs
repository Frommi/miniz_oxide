...
fuzz_target!(|data: &[u8]| {
    ...
    assert_eq!(
        rust_compressed_buf[..rust_compressed_size as usize],
        c_compressed_buf[..c_compressed_size as usize]
    );

    let rust_res = unsafe {
        miniz_oxide_c_api::mz_uncompress(
            rust_decompressed_buf.as_mut_ptr(),
    ...
    assert_eq!(
        rust_decompressed_buf[..rust_decompressed_size as usize],
        c_decompressed_buf[..c_decompressed_size as usize]
    );

    assert_eq!(rust_decompressed_size, uncompressed_size);
    assert_eq!(
        rust_decompressed_buf[..c_decompressed_size as usize],
